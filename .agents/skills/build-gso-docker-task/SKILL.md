...
- Treat commit analysis, LLM generation, Docker image builds, generation-time test execution, and formal performance execution as potentially long-running or costly. For a request to create/configure the skill or explain commands, do not start those stages. For an explicit request to build a task, proceed stage by stage and report material failures. Prefer backgrounding hour-scale generation and execution with `nohup ... > log 2>&1 &` + `tail -f` (see [references/workflow.md](references/workflow.md)) so terminal disconnects do not abort a run mid-stream.
...
- `tests_per_commit`: default `5`. Pass `--n 5` to generation unless the user explicitly requests another count; treat five as the target, not an all-or-nothing requirement.
...
5. Before generation, inspect the repository's packaging and bootstrap files and decide whether the YAML `install_commands` will install the repository and required runtime dependencies. If repository-specific commands are needed, update the YAML first. If the YAML omits `install_commands` and the standard `Problem` commands are sufficient, leave the field absent. Also inspect the base image, the local checkout's `.git`, and candidate plus parent commit availability: generation now requires Docker and prepares the repository image before its first LLM request.
    ...
    **`install_commands` must not move the shell's working directory off the repository root.** The phase1/phase2 scripts run `cd $repo_name`, then the install commands, then `cd ..` to return to `/workspace` before globbing `<hash>/test_*.py`; a bare `cd` inside the install commands (for example into a `packages/` subdirectory) makes that final `cd ..` land in the wrong directory, so the test glob matches nothing and the container logs fail as `//<hash>/test_*.py`. When a `cd` is unavoidable, wrap it in a subshell `( cd subdir && ... )` so the parent shell cwd is preserved. The standard `Problem` install commands (`src/gso/data/problem.py`) contain no `cd` by design — that is the contract's baseline. See [references/workflow.md](references/workflow.md) for the exact contract, common violations, and a local check.
6. Generate only after those checks. Pass `--n 5`, the same `--max_year 2022` cutoff used for analysis, the local analysis checkout with `--docker-repo-path`, plus the intended image, base image, and platform. By default generate across all mapped APIs and all eligible commits; use `--api` only when the user requests one API or when explicitly iterating cheaply. For each commit, target five tests and construct them one at a time. Run each proposed test against the parent and candidate in Docker and supply validation failures to up to three semantic retries. If a test slot still fails after all three retries, record that slot's diagnostics, skip it, and continue constructing the next test slot; never terminate the commit or task solely because one test could not be produced. Only successfully executed scenarios inform later requests, and a commit may finish with fewer than five accepted tests. Verify `${exp_id}_problems.json`, generation-validation logs, and any retry/failure diagnostic JSON. A task-level generation failure does not replace an existing valid problems file. The phase scripts hardcode `timeout 300s` per iteration and repeat each run 3× in phase 2, so set the YAML `repo_instr` to bound the base/parent `experiment` run to ~30–120 s (ideally ~60 s) — speedup is a ratio, so a ~60 s base run is enough to show a large speedup and will not risk the 300 s ceiling. Estimate the `--n` × commits × APIs × ~7 timed iterations matrix before launching; when it exceeds an hour, scope with `--api` or lower `--n`. Run generation in the background (`nohup ... > log 2>&1 &` + `tail -f`) so a dropped terminal cannot kill an hour-scale run (see [references/workflow.md](references/workflow.md) §4).
...
- Generated scenario/test rejection: inspect `${exp_id}_generation_retries_<timestamp>.json` for retried or skipped test slots and `${exp_id}_generation_failed_<timestamp>.json` for task-level terminal failures, then correlate `execution_log` with `docker_logs/generation_validation/`. Confirm that exhausting three retries for one slot did not prevent later slots from being attempted. Distinguish response-format/static-validation errors from setup, reference, candidate, or equivalence failures.
...
- No usable generation or execution results: inspect the task-specific directory under `docker_logs/`. A log line `python: can't open file '//<hash>/test_*.py'` means `install_commands` left the shell cwd off the repository root, so phase1's `cd ..` landed on `/` and the test glob matched nothing (see the install_commands working-directory contract in [references/workflow.md](references/workflow.md)); verify the joined install commands restore cwd to the repo root. To confirm the on-disk layout directly, re-run the failing API's generation with `--keep-workspaces` (scoped with `--api`) and inspect the retained `phase1.sh`/`phase2.sh` and `<quick_hash>/test_*.py` instead of reasoning from the container log alone ([references/workflow.md](references/workflow.md) §4); otherwise validate generated test determinism, phase scripts, and resource limits.
