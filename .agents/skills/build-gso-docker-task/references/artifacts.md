...
- Generation targets five tests per commit by default (`--n 5`) but may retain fewer. Exhausting three semantic retries for one test slot must be recorded in `${exp_id}_generation_retries_<timestamp>.json` and must not prevent later slots from being attempted. Task-level terminal failures write `${exp_id}_generation_failed_<timestamp>.json`. Inspect attempt `error` and `execution_log` fields without treating raw model output as a valid test.
