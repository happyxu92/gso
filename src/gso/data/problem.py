...
class Tests(BaseModel):
    ...
    def add_sample(self, test: str):
        if not isinstance(test, str) or not test.strip():
            raise ValueError("Cannot add an empty generated performance test")
        self.samples.append(test + TIMEIT_TEMPLATE + TEST_HARNESS)

    def add_samples(self, samples: list[str]):
        if any(not isinstance(test, str) or not test.strip() for test in samples):
            raise ValueError("Cannot add an empty generated performance test")
        self.samples.extend([test + TIMEIT_TEMPLATE + TEST_HARNESS for test in samples])
