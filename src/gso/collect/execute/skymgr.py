...
class SkyManager:
    """Generate and manage SkyPilot tasks for performance testing."""

    cluster_prefix = "sky-gso"

    def validate(self, problems=None):
        if shutil.which("sky") is None:
            raise RuntimeError(
                "SkyPilot CLI not found. Install it or use --backend docker."
            )
    ...
    def launch_task(task_yaml, workspace, cluster="sky-gso", interactive=False):
        ...
        if not interactive:
            ...
            subprocess.run(cmd, cwd=workspace, text=True, check=True)
    ...
    def get_results(workspace, cluster="sky-gso"):
        ...
        results = collect_results(workspace / "results")
        message = (
            f"Cluster {cluster}: collected {len(results)} result(s)"
            if results
            else f"Cluster {cluster}: no results"
        )
        return message, results

    @staticmethod
    def get_exit_code(cluster):
        # SkyPilot status does not expose a process-style exit code here.
        return None
