...
from gso.collect.pids import (
    TEST_PROBLEMS as DEFAULT_TEST_PROBLEMS,
    LONG_RUNNING_PROBLEMS as DEFAULT_LONG_RUNNING_PROBLEMS,
)
from gso.collect.utils import prepare_prob_script, prepare_install_commands


def load_pids_config(
    pids_file: str | None,
) -> tuple[dict[str, list[tuple[str, str]]], list[tuple[str, str, int]]]:
    """Load task selections without modifying the repository's default pids.py."""
    if pids_file is None:
        return DEFAULT_TEST_PROBLEMS, DEFAULT_LONG_RUNNING_PROBLEMS

    path = Path(pids_file).expanduser().resolve()
    if not path.is_file():
        raise FileNotFoundError(f"PIDs file not found: {path}")

    namespace = run_path(str(path))
    test_problems = namespace.get("TEST_PROBLEMS")
    long_running_problems = namespace.get("LONG_RUNNING_PROBLEMS", [])
    if not isinstance(test_problems, dict) or not test_problems:
        raise ValueError(f"{path} must define a non-empty TEST_PROBLEMS dictionary")
    if not isinstance(long_running_problems, list):
        raise ValueError(f"{path}: LONG_RUNNING_PROBLEMS must be a list")

    normalized: dict[str, list[tuple[str, str]]] = {}
    for exp_id, entries in test_problems.items():
        if not isinstance(exp_id, str) or not isinstance(entries, (list, tuple)):
            raise ValueError(
                f"{path}: TEST_PROBLEMS must map experiment names to task lists"
            )
        normalized_entries = []
        for entry in entries:
            if (
                ...
                raise ValueError(
                    f"{path}: invalid task {entry!r}; expected (pid, commit)"
                )
            normalized_entries.append((entry[0], entry[1]))
        normalized[exp_id] = normalized_entries

    print(f"Loaded task selections from {path}")
    return normalized, long_running_problems
...
def build_dataset(
    ...
    long_running_problems = (
        DEFAULT_LONG_RUNNING_PROBLEMS
        if long_running_problems is None
        else long_running_problems
    )
...
def main(
    ...
    if results_file is not None and exp_id is None:
        raise ValueError("--results-file requires --exp_id")

    test_problems, long_running_problems = load_pids_config(pids_file)
    exp_ids = list(test_problems) if exp_id is None else [exp_id]
    results_paths = [
    ...
    if missing_paths:
        ...
        raise FileNotFoundError(f"Execution results not found:\n{formatted_paths}")

    print(f"Loading {backend} execution results:")
    for path in results_paths:
        print(f"  {path}")
    problems = [
        problem
        for results_path in results_paths
        for problem in load_problems(results_path)
    ]

    if exp_id and exp_id not in test_problems:
        exp_id = None  # unset
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["sky", "docker"],
        default="sky",
        help="Execution result backend (default: sky)",
    )
    parser.add_argument(
        "--results-file",
        default=None,
